...
```{r fig.height=2.5, fig.width=5.25}
df %>%
  filter(significant == "p < 0.05") %>%
  ...
  facet_wrap(~genotype, nrow = 1, strip.position = "bottom") +
  ...
  theme(legend.key.height = unit(0, "cm"), plot.margin = unit(c(0.5,0.5,0,0), "lines"))
...
df.venn <- data.frame(x = c(0, 0.866, -0.866),
                      y = c(1, -0.5, -0.5),
                      labels = c('Sgca-null', 'IC Sgcb-null', 'ID Sgcb-null'))
